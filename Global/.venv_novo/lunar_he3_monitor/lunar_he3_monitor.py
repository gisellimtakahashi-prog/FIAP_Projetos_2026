...
# Configurações globais de renderização e tamanho de fontes
...
def build_advanced_dashboard(
    ...
    # SUGESTÃO 1 APLICADA: Preenchimento de área para melhor percepção de magnitude
    ax1.fill_between(
        short_times,
        df_plasma["velocity_km_s"],
        color="#e74c3c",
    ...
    # Plot 2: Campo Magnético Interplanetário Bz (Norte vs Sul)
    ax2 = fig.add_subplot(gs[1, 0])
    bz = df_mag["bz_nt"].values

    ax2.plot(short_times, bz, color="#7f8c8d", linestyle="-", alpha=0.4, zorder=1)

    pos_mask = bz >= 0
    neg_mask = bz < 0
    st_array = np.array(short_times)

    ax2.scatter(st_array[pos_mask], bz[pos_mask], color="#3498db", marker="o", s=50, label="Bz $\geq$ 0 (Norte)",
                zorder=3)
    ax2.scatter(st_array[neg_mask], bz[neg_mask], color="#e67e22", marker="o", s=50, label="Bz < 0 (Sul)", zorder=3)

    # SUGESTÃO 2 APLICADA: Faixas de fundo dinâmicas para contextualizar o fenômeno físico
    ax2.axhspan(0, max(bz) + 2, color='#3498db', alpha=0.04, zorder=0)
    ax2.axhspan(min(bz) - 2, 0, color='#e67e22', alpha=0.04, zorder=0)

    ax2.axhline(0, color="black", linestyle="--", alpha=0.5, zorder=2)
    ax2.set_title(
        r"Componente IMF $B_z$ (Fundo Azul: Norte / Fundo Laranja: Sul)",
        weight="bold",
    )
    ...
    # SUGESTÃO 3 APLICADA: Adicionar métricas do modelo e equação diretamente no gráfico
    r2_val = r2_score(df_apollo["He3_ppb"].values, model.predict(df_apollo[["TiO2_Percent"]].values))
    text_metrics = f'$R^2 = {r2_val:.3f}$\n$He_3 = {model.coef_[0]:.2f} \cdot TiO_2 + {model.intercept_:.2f}$'
    box_properties = dict(boxstyle='round', facecolor='#f39c12', alpha=0.15, edgecolor='#d35400')
    ax3.text(0.05, 0.93, text_metrics, transform=ax3.transAxes, fontsize=8.5, verticalalignment='top',
             bbox=box_properties)
